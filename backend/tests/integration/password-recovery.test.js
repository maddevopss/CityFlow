...
jest.mock('../../src/api/middleware/rateLimiters', () => ({ loginLimiter: (req, res, next) => next() }));
...
describe('password recovery', () => {
  ...
  afterAll(clearSmtpEnvironment);

  test('retourne une réponse générique pour un utilisateur inexistant', async () => {
    mockPrisma.user.findUnique.mockResolvedValue(null);
    const response = await request(createApp()).post('/forgot-password').send({ email: 'absent@example.invalid' });
    expect(response.status).toBe(202);
    expect(response.body.message).toBe('Si un compte correspond, les instructions ont été envoyées');
  ...
  test('utilise SMTP2GO et génère un jeton sans donnée du mot de passe', async () => {
    ...
    mockPrisma.user.findUnique.mockResolvedValue({
      ...
      password: '$2a$12$secret-hash-that-must-not-enter-token',
      isActive: true,
      updatedAt
    });
    mockSendMail.mockResolvedValue({ messageId: 'test-message' });

    const response = await request(createApp()).post('/forgot-password').send({ email: 'active@example.invalid' });
    expect(response.status).toBe(202);
    expect(mockCreateTransport).toHaveBeenCalledWith({
      host: 'mail.smtp2go.com',
      port: 587,
      secure: false,
      auth: { user: 'smtp2go-test-user', pass: 'smtp2go-test-password' }
    });
  ...
  test('réinitialise le mot de passe avec une version utilisateur valide', async () => {
    const updatedAt = new Date('2026-08-03T12:00:00.000Z');
    const user = { id: 'f2ef70de-e292-48d4-bd94-ec50d2f5ecab', isActive: true, updatedAt };
    ...
    const response = await request(createApp()).post('/reset-password').send({
      ...
      passwordConfirmation: 'NouveauMotDePasse-2026'
    });
