...
test('crée un ordre de travail isolé par municipalité', async () => {
  const db = { $executeRawUnsafe: jest.fn().mockResolvedValue(1), $queryRawUnsafe: jest.fn().mockResolvedValueOnce([]).mockResolvedValueOnce([{ id: 'order-1', municipalityId: 7, status: 'DRAFT' }]) };
  const result = await createWorkOrder(db, { municipalityId: 7, title: 'Réparer', description: 'Réparer le trottoir', actorId: '11111111-1111-4111-8111-111111111111' });
  expect(result).toMatchObject({ municipalityId: 7, status: 'DRAFT' });
