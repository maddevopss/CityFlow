...
async function assignWorkOrder(db, { municipalityId, workOrderId, teamId, vehicleId, actorId }) {
  if (!teamId) {
    const error = new Error('team required');
  ...
  await db.$executeRawUnsafe(
    ...
    teamId, vehicleId || null, workOrderId, municipalityId
  );
  await db.$executeRawUnsafe(
    ...
    randomUUID(), municipalityId, workOrderId, actorId, order.status, JSON.stringify({ teamId, vehicleId: vehicleId || null })
  );
