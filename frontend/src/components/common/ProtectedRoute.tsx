...
const roleHierarchy: Record<AppRole, number> = {
  ...
  PERMIT_REVIEWER: 3,
  ASSET_MANAGER: 3,
  PUBLIC_WORKS_MANAGER: 3,
  CITIZEN_SERVICE_AGENT: 3,
  EXECUTIVE_VIEWER: 3,
  MUNICIPAL_AGENT: 3,
  MUNICIPAL_MANAGER: 4,
  ADMIN: 5
...
const ProtectedRoute: React.FC<ProtectedRouteProps> = ({ requiredRole = 'VIEWER', children }) => {
  ...
  const userLevel = user ? roleHierarchy[user.role as AppRole] ?? -1 : -1;
  if (userLevel < roleHierarchy[requiredRole]) return <Navigate to="/" replace />;
  return <>{children}</>;
};
