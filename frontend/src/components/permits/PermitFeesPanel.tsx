...
import { assessPermitFee, getPermitFee, markPermitFeePaid } from '../../services/permitService';
...
const PermitFeesPanel: React.FC<Props> = ({ permitId, role }) => {
  ...
  return <section className="space-y-3 rounded-lg border border-gray-200 p-4" aria-labelledby="permit-fees-title">
    <div className="flex items-start justify-between gap-3">
      <div><h3 id="permit-fees-title" className="font-semibold text-gray-900">Frais et paiement</h3><p className="text-sm text-gray-500">Suivi municipal du montant exigé et du paiement externe.</p></div>
      {feeQuery.data ? <span className={`rounded-full px-2 py-1 text-xs font-semibold ${feeQuery.data.status === 'PAID' ? 'bg-green-100 text-green-800' : 'bg-amber-100 text-amber-800'}`}>{feeQuery.data.status === 'PAID' ? 'Payé' : feeQuery.data.status === 'WAIVED' ? 'Dispensé' : 'À payer'}</span> : null}
    </div>
    ...
    {feeQuery.data ? <dl className="grid grid-cols-2 gap-3 text-sm"><div><dt className="text-gray-500">Montant</dt><dd className="font-semibold text-gray-900">{formatMoney(feeQuery.data.amountCents, feeQuery.data.currency)}</dd></div><div><dt className="text-gray-500">Référence</dt><dd className="break-all">{feeQuery.data.paymentReference || '—'}</dd></div>{feeQuery.data.note ? <div className="col-span-2"><dt className="text-gray-500">Note</dt><dd>{feeQuery.data.note}</dd></div> : null}</dl> : !feeQuery.isLoading ? <p className="text-sm text-gray-500">Aucun frais évalué.</p> : null}
    {canWrite && feeQuery.data?.status !== 'PAID' ? <form onSubmit={(event) => { event.preventDefault(); setMessage(null); assessMutation.mutate(); }} className="grid gap-2 rounded-lg bg-gray-50 p-3 text-sm">
    ...
    {canWrite && feeQuery.data?.status === 'DUE' ? <form onSubmit={(event) => { event.preventDefault(); if (paymentReference.trim().length < 3) { setMessage('La référence de paiement doit contenir au moins 3 caractères.'); return; } setMessage(null); paymentMutation.mutate(); }} className="grid gap-2 rounded-lg bg-gray-50 p-3 text-sm">
