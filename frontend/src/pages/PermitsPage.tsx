...
const actionLabels: Record<PermitTransitionAction, string> = {
  submit: 'Soumettre',
  approve: 'Approuver',
  reject: 'Refuser',
  close: 'Fermer'
...
function availableActions(status: PermitStatus, role?: string): PermitTransitionAction[] {
  ...
  const canOperate = canReview || role === 'MUNICIPAL_AGENT';
...
const PermitsPage: React.FC = () => {
  ...
  const [pendingAction, setPendingAction] = useState<PermitTransitionAction | null>(null);
  ...
  const transitionMutation = useMutation({
    mutationFn: ({ permitId, action, reason: transitionReason }: { permitId: string; action: PermitTransitionAction; reason?: string }) =>
      transitionPermit(permitId, action, transitionReason),
    onSuccess: async (_, variables) => {
      setActionMessage(`Action « ${actionLabels[variables.action]} » effectuée.`);
  ...
  const executeAction = () => {
    ...
    const needsReason = pendingAction === 'reject' || pendingAction === 'close';
    if (needsReason && reason.trim().length < 3) {
      setActionMessage('Un motif d’au moins 3 caractères est requis.');
      return;
    }
    setActionMessage(null);
    transitionMutation.mutate({ permitId: selectedId, action: pendingAction, reason: needsReason ? reason.trim() : undefined });
  };
  ...
  return (
    <main className="space-y-6 p-4 md:p-6">
      <header>
        ...
        <p className="mt-1 text-sm text-gray-600">Consultez les permis, leur état, leur historique, les inspections et les décisions municipales.</p>
      </header>
            ...
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                ...
                {permitsQuery.data?.items.map((permit) => (
                  <tr key={permit.id} onClick={() => { setSelectedId(permit.id); setPendingAction(null); setActionMessage(null); }} className={`cursor-pointer hover:bg-cityflow-50 ${selectedId === permit.id ? 'bg-cityflow-50' : ''}`}>
          ...
          {detailQuery.data ? (
            <div className="space-y-6">
              ...
              {permitActions.length > 0 ? (
                <section className="rounded-lg border border-cityflow-200 bg-cityflow-50 p-4" aria-label="Décision sur le permis">
                  <h3 className="font-semibold text-gray-900">Actions disponibles</h3>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {permitActions.map((action) => <button key={action} type="button" disabled={transitionMutation.isPending} onClick={() => { setPendingAction(action); setReason(''); setActionMessage(null); }} className="rounded-lg bg-cityflow-700 px-3 py-2 text-sm font-medium text-white disabled:opacity-50">{actionLabels[action]}</button>)}
                  </div>
                  {pendingAction ? (
                    <div className="mt-4 space-y-3">
                      <p className="text-sm font-medium">Confirmer : {actionLabels[pendingAction]}</p>
                      {(pendingAction === 'reject' || pendingAction === 'close') ? <label className="block text-sm text-gray-700">Motif obligatoire<textarea value={reason} onChange={(event) => setReason(event.target.value)} rows={3} maxLength={1000} className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2" /></label> : null}
                      <div className="flex gap-2"><button type="button" onClick={executeAction} disabled={transitionMutation.isPending} className="rounded-lg bg-gray-900 px-3 py-2 text-sm font-medium text-white disabled:opacity-50">{transitionMutation.isPending ? 'Traitement…' : 'Confirmer'}</button><button type="button" onClick={() => { setPendingAction(null); setReason(''); }} className="rounded-lg border border-gray-300 px-3 py-2 text-sm">Annuler</button></div>
                    </div>
                  ) : null}
                  {actionMessage ? <p role="status" className="mt-3 text-sm text-gray-700">{actionMessage}</p> : null}
                </section>
              ) : null}

              <section><h3 className="font-semibold text-gray-900">Historique</h3><ol className="mt-2 space-y-2">{detailQuery.data.history.map((entry) => <li key={entry.id} className="rounded-lg bg-gray-50 p-3 text-sm"><span className="font-medium">{entry.action}</span><span className="block text-gray-500">{entry.previousStatus || '—'} → {entry.newStatus || '—'} · {formatDate(entry.occurredAt)}</span>{entry.reason ? <span className="mt-1 block text-gray-700">Motif : {entry.reason}</span> : null}</li>)}</ol>{detailQuery.data.history.length === 0 ? <p className="mt-2 text-sm text-gray-500">Aucun historique disponible.</p> : null}</section>
