...
**Correctif appliqué (2026-08-05, sur demande explicite)** : la note a été ajoutée dans `docs/integration-systeme-mad/README.md`, sous la section « Source institutionnelle épinglée ». Le nom épinglé dans le manifeste n'a pas été changé (il reste la référence de l'examen initial et `validate-systeme-mad-alignment.mjs` continue de l'exiger) ; seule la note d'orientation a été ajoutée. Vérifié : `node scripts/validate-systeme-mad-alignment.mjs` → toujours SUCCÈS.
...
**Correctif appliqué (2026-08-05, sur demande explicite)** : `docs/traceability/risks.json` était le principal reliquat après la clôture de C1/C2/C3 dans les registres T (les registres Markdown portaient déjà la trace, mais le catalogue JSON — la seule source que `scripts/validate-traceability.mjs` vérifie automatiquement — ne tracait toujours qu'une exigence de gouvernance auto-référentielle). Une nouvelle exigence réelle a été ajoutée (`REQ-CF-SEC-001` : « un changement de sécurité ne doit dégrader ni la disponibilité de l'API ni le filet de tests qui la protège ») avec sa chaîne complète — décision (`DEC-CF-SEC-001`), les trois risques (`RSK-2026-001/002/003`, statut `treated`), le changement (`CHG-2026-001`), le test (`TST-2026-001`), la preuve (`PRV-2026-001`) et la validation (`VAL-2026-001`) — plutôt que de rattacher artificiellement ces risques à l'exigence de traçabilité existante, à laquelle ils n'ont pas de rapport sémantique. Vérifié : `node scripts/validate-traceability.mjs` → SUCCÈS (2 exigences, 17 liens, 100 % de couverture obligatoire). `docs/registres/t1-registre-decisions.md` reste, lui, à l'état de coquille : le peupler avec de vraies décisions produit (pas seulement de sécurité) dépasse le périmètre de ce correctif et reste une action de suivi.
